...
export default function AddEmployeePage() {
  ...
  const { data: employees = [], isLoading: employeesLoading } = useQuery<
    EmployeeLite[]
  >({
    queryKey: ["employee-form", "employees"],
    queryFn: async () => {
  ...
  const createEmployeeMutation = useMutation({
    mutationFn: async () => {
      ...
      console.log("create employee", fd);
      console.log("employee json", fd.get("employee"));
  ...
  const initialLoading =
    departmentsLoading || designationsLoading || employeesLoading;
        ...
        >
          ...
          <Field label="About">
            <textarea
              name="about"
              onChange={handleChange}
              className="border rounded-lg input h-24"
            />
          </Field>
        </form>
      </div>
