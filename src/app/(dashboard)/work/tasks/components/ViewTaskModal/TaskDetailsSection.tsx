    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-6 border-b">
      ...
      <Card className="p-6 rounded-2xl border-slate-200 shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-y-4 text-sm">
          ...
          <p className="text-slate-700">{task.milestone?.title || "--"}</p>

          <p className="text-slate-500">Label</p>
          <p className="text-slate-700">{labels}</p>

          <p className="text-slate-500">Task Category</p>
          <p className="text-slate-700">{task.categoryId?.name || "--"}</p>
      ...
      <Card className="p-6 rounded-2xl border-slate-200 shadow-sm">
        <div className="flex items-center gap-2 mb-4">
          <span
            ...
            style={{
              backgroundColor: task?.taskStage?.labelColor || "#3b82f6",
            }}
          />
          <p className="font-medium text-slate-700">
            {task?.taskStage?.name || "uuuuu"}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-y-4 text-sm">
          ...
          <p className="text-slate-700">{formatShort(task.dueDate) || "--"}</p>

          <p className="text-slate-500">Hours Logged</p>
          <p className="text-slate-700">{task.hoursLogged || 0}h</p>
        </div>
      </Card>
    </div>
  );
